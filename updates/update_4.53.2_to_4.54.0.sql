...
-- Adicionar campo email_contador para armazenar o e-mail do contador da empresa
ALTER TABLE `emitente` ADD COLUMN `email_contador` VARCHAR(255) NULL DEFAULT NULL AFTER `email`;

-- Adicionar campo codigo_ibge para o código do município
ALTER TABLE `emitente` ADD COLUMN `codigo_ibge` VARCHAR(10) NULL DEFAULT NULL AFTER `cep`;

-- Adicionar campos para controle de numeração de NFe e NFCe
ALTER TABLE `emitente` ADD COLUMN `numero_nfe` INT(11) NOT NULL DEFAULT 0 AFTER `codigo_ibge`;
ALTER TABLE `emitente` ADD COLUMN `numero_nfce` INT(11) NOT NULL DEFAULT 0 AFTER `numero_nfe`;

-- Adicionar campos de latitude e longitude
ALTER TABLE `emitente` ADD COLUMN `latitude` VARCHAR(50) NULL DEFAULT NULL;
ALTER TABLE `emitente` ADD COLUMN `longitude` VARCHAR(50) NULL DEFAULT NULL;